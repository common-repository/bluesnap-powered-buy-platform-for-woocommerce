...
class Fisha_Bsnp_Api {
	...
	public function get_bsnp_error_code( $response ) {
		if ( isset( $response['message']['code'] ) ) {
			...
			if(isset( $response['message'][0]['code']) ){
				$bsnp_error_code = $response['message'][0]['code'];

				return $bsnp_error_code;
			}else{
				if(isset( $response['message'][1]['code']) ){
					$bsnp_error_code = $response['message'][1]['code'];
	...
	public function get_bsnp_error_msg( $err_code ) {
		$bsnp_general_error_code = __( "Your payment could not be processed at this time. Please make sure the card information was entered correctly and resubmit. If the problem persists, please contact your credit card company to authorize the purchase.",
		...
		switch ( $err_code ) {
			case '10000':
				...
				break;
			case '10001':
				$error_string = __("Your payment could not be processed at this time as this card brand is not supported. Please select a new payment method and try again",
                    "woocommerce-bluesnap-powered-buy-platform-for-woocommerce");
				break;
			case '14001':
				$error_string = $bsnp_general_error_code;
			...
			case '14002':
				...
				break;
			case '15003':
				$error_string = __( "The currency selected is not supported by this payment method. Please try a different currency or payment method",
                    "woocommerce-bluesnap-powered-buy-platform-for-woocommerce");
				break;
			/**
