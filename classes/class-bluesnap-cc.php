...
class WC_Gateway_Bluesnap_Cc extends WC_Payment_Gateway {
	...
	public function __construct() {
		...
		if ( isset( WC()->session ) ) {
			$this->bsnp_fruad_session = WC()->session->get( 'fruad_session', null );

			if ( $this->bsnp_fruad_session == null ) {
				$this->bsnp_fruad_session = $GLOBALS['FiSHa_BSNP_Functions']->gen_uuid();
				WC()->session->set( 'fruad_session', $this->bsnp_fruad_session );
			}
		} else {
			$this->bsnp_fruad_session = $GLOBALS['FiSHa_BSNP_Functions']->gen_uuid();
		}
...
//        add_action( 'woocommerce_email_customer_details', array($GLOBALS['FiSHa_BSNP_Currency_Switcher'], 'add_email_order_meta'), 10, 2 );
	...
	public function credit_card_form( $args = array(), $fields = array() ) {
		...
		$default_fields     = array_merge(
			...
			array(
                ...
                'fraud-session'        => '<input type="hidden"  name="fraud_session" value="'.$this->bsnp_fruad_session.'">',
	...
	public function process_payment( $order_id ) {
		...
		if ( $response ) {
			$usr_id  = $this->customer_order->get_user_id();
		    ...
		    if ( ( ! $this->bsnp_is_preorder ) && ( ! $this->bsnp_is_subsacription ) ) {
				if ( ! $GLOBALS['FiSHa_BSNP_Functions']->bsnp_is_returnig_shopper() ) {
					...
					$GLOBALS['FiSHa_BSNP_Db']->bsnp_create_return_shopper( $this->bsnp_shopper_id, $usr_id );
				...
				$GLOBALS['FiSHa_BSNP_Functions']->bsnp_save_bsnp_order( $this->bsnp_order_id, $this->bsnp_invoice_id, $order_id, $this->bsnp_shopper_id, $usr_id );
			}
			if ( $this->bsnp_is_preorder || $this->bsnp_is_subsacription ) {
			...
			$GLOBALS['FiSHa_BSNP_Currency_Switcher']->bsnp_keep_exchange_rate( $order_id, $this->purchase_currency, $ex_rate );
			if ( ! $this->bsnp_is_preorder && ! $this->bsnp_is_subsacription ) {
				...
				$GLOBALS['FiSHa_BSNP_Subscriptions']->bsnp_save_subscription( $this->bsnp_order_id, $order_id  );
			...
			add_user_meta( $usr_id, '_bsnp_approved_shopper', 1, true );
			if(!empty($ex_rate)){
			    update_post_meta($order_id  , '_bsnp_ex_rate', $ex_rate );
			}
...
//			}
	...
	public function bsnp_set_is_preorder( $order_id ) {
		if ( class_exists( 'WC_Pre_Orders_Order' ) ) {
			...
			//if order is pay later charge on release as regular order
...
//                                    'card-type' => strtoupper( sanitize_text_field( $_POST['credit-card-type'] ) ),
			...
			'order'   => array(
				'ordering-shopper'     => array(
					'fraud-info' => array( 'fraud-session-id' => $_POST['fraud_session'] ),
					'web-info'   => array(
						'ip'          => $customer_order->customer_ip_address,
