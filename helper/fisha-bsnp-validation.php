...
class Fisha_Bsnp_Validation {
	 ...
	 * @return string
	 */
	public function country_shipping_states_approved( $order ) {

		$shipping_state = version_compare( WC_VERSION, '3.0.0', '<' )
			? $order->shipping_state : $order->get_shipping_state();

		$shipping_country = version_compare( WC_VERSION, '3.0.0', '<' )
			? $order->shipping_country : $order->get_shipping_country();

		if ( ! in_array( $shipping_country, $this->list_of_countries_with_states ) ) {
			$shipping_state = '';
		}
	 ...
	 * @return string
	 */
	public function country_billing_states_approved( $order ) {
		$billing_state = version_compare( WC_VERSION, '3.0.0', '<' )
			? $order->billing_state : $order->get_billing_state();

		$billing_country = version_compare( WC_VERSION, '3.0.0', '<' )
			? $order->billing_country : $order->get_billing_country();

		if ( ! in_array( $billing_country, $this->list_of_countries_with_states ) ) {
			$billing_state = '';
		}
